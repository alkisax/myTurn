...
public class TicketServiceSecurityTests
{
  ...
  private static async Task<int> CreateService(Setup a) => await Id(await Send(HttpMethod.Post, "/services/", a.Token, new { locationId = a.LocationId, name = "service_" + Guid.NewGuid().ToString("N"), description = "test", isGeneric = false, estimatedServiceMinutes = 5 }));
